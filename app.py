...
if uploaded_file is not None:
    ...
    if ret:
        ...
        if value is not None:
            ...
            # --- UNIVERSAL HYBRID DETECTION (v7.0) ---
            ...
            # Find all local maxima (the bottom-most points of the bar)
            potential_bottoms = []
            ...
            rep_data = []
            for start in potential_bottoms:
                # Check for Squat vs Deadlift: did it move >20px down in the second before?
                pre_range = smoothed_y[max(0, start-int(fps)):start]
                is_squat = (max(pre_range) - min(pre_range)) > 20 if len(pre_range) > 0 else False
                
                # Search for Lockout
                search_range = smoothed_y[start:min(start + int(fps * 3), len(smoothed_y))]
                end = start
                for j in range(2, len(search_range)):
                    if search_range[j] >= search_range[j-1]: # The moment upward travel stalls
                        end = start + j
                        break
                
                # Validation: Rise must be > 20cm
                total_rise = (smoothed_y[start] - smoothed_y[end]) * meters_per_pixel
                if total_rise > 0.20:
                    # Snap to absolute bottom for Squats
                    actual_start = smoothed_y.index(max(smoothed_y[start-10:start+10])) if is_squat else start
                    
                    if not rep_data or (actual_start - rep_data[-1]['end']) > (fps * 0.5):
                        rep_y = y_history[actual_start:end+1]
                        v_raw = [abs(rep_y[k-1] - rep_y[k]) * meters_per_pixel * fps for k in range(1, len(rep_y))]
                        
                        rep_data.append({
                            "id": len(rep_data) + 1,
                            "start": actual_start, "end": end,
                            "avg_v": np.mean(v_raw), "peak_v": max(v_raw),
                            "duration": (end - actual_start) / fps
                        })
            ...
            with col1:
                with st.spinner("Baking Pro-Grade Video..."):
                    ...
                    for i in range(len(all_frames)):
                        ...
                        if active_rep:
                            cur_t = (i - active_rep['start']) / fps
                            cv2.putText(frame_draw, f"REP {active_rep['id']} | {cur_t:.2f}s", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    ...
                    if rep_data:
                        ...
                        y_off = 140
                        for r in rep_data:
                            cv2.putText(summary_frame, f"REP {r['id']}: {r['duration']:.2f}s | {r['avg_v']:.2f}m/s", (40, y_off), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                            y_off += 50
