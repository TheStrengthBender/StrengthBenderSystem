...
# Only run this if the button has been pressed
if st.session_state.analysis_active and uploaded_file is not None:
    ...
    if ret:
        ...
        st.write("### 🎯 Step 1: Click the Barbell")
        # The key stays consistent so it triggers the calculation on click
        value = streamlit_image_coordinates(cv2.cvtColor(first_frame_res, cv2.COLOR_BGR2RGB), key="barbell_click")

        if value:
            with st.status("🎯 Target Locked. Processing Physics...", expanded=True) as status:
                ...
                for i in range(len(frames)):
                    ...
                    if curr:
                        ...
                        cv2.putText(img, "THE STRENGTH BENDER", (10, 35), 0, 0.5, (150,150,150), 1)
